...
public class Controleur implements Observer {
    ...
    public void update(Observable o, Object arg) {
        ...
        switch (m.type) {
            case CLIC_BTN_ALLER:
                if(action < avCourant.getActionsMax()){
            ...
            if(this.action >= avCourant.getActionsMax() && 
              !(this.doubleAssechement && avCourant.getTuilesAssechables(grille).size() > 0)){
            this.finTour();
            if(this.action >= avCourant.getActionsMax()){
                this.finTour();
            }
    ...
    public void ajouterAction() {
        ...
        if (action >= avCourant.getActionsMax() && !doubleAssechement) {
            finTour();
        }
        this.vueAventurier.setNbAct(avCourant.getActionsMax() - action);
        }
    ...
    private void traiterClicCase(int x, int y) {
        if (operationEnCours == OPERATION_DEPLACEMENT && 
            ((this.action < avCourant.getActionsMax()))) {
    ...
    private void removeCarteSacsDeSable() {
        for(CarteTresor c : avCourant.getCartes()){
            if(c.getTypeCarte().equals("action_speciale")){
                CarteBonus d = (CarteBonus) c;
